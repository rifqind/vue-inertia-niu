export function getPagination(
    tabelDinas, currentPagination, maxRows, statusText,
    ...
    var trnum = 0
        ...
        if (rowsTabel > currentMaxRows) {
            ...
            for (let i = 1; i <= totalPageNum;) {
                ...
                currentPagination.value.insertBefore(li, document.getElementById("prev"));
            }
        }
        var lastPage = 0
        ...
        listLi.forEach(function (list) {
            list.addEventListener("click", function (e) {
                ...
                listLi.forEach(function (e, index) {
                    if (index !== 0 && index !== currentPagination.value.children.length - 1) {
                        e.classList.remove('active')
                    }
                })
                currentPagination.value.querySelector("[data-page='" + lastPage + "']").classList.add('active')
                tabelDinas.value.querySelectorAll('tbody tr').forEach(function (e) {
                    trIndex++
                    if (trIndex > currentRows * currentPageNum || trIndex <= currentRows * currentPageNum - currentRows) {
        ...
        let activePage = parseInt(currentPagination.value.querySelector('.active').getAttribute('data-page'))
        if (currentPagination.value.querySelectorAll('li').length > 7) {
            if (activePage <= 3) {
                currentPagination.value.querySelectorAll('li').forEach(function (e, index) {
                    if (index > 5) {
                        e.style.display = 'none'
                    } else {
                        e.style.display = ''
                ...
                currentPagination.value.querySelector('[data-page="next"]').style.display = 'block'
            }
            if (activePage > 3) {
                currentPagination.value.querySelectorAll('li').forEach(function (e, index) {
                    if (index > 0) {
                ...
                for (let i = activePage - 2; i <= activePage + 2; i++) {
                    currentPagination.value.querySelector("[data-page='" + i + "']").style.display = 'block'
                }
            }
        }
}