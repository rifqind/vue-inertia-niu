...
watch(allColumnSelected, (value) => {
  allColumnFinal.value = allColumnTempt.value.filter((item) =>
...
watch(
  () => form.rows.tipe,
  (value) => {
    if (!value) rowListFetched.value = [];
    tingkatanDrop.value.value = null;
    orderDropRow.value = 2;
    rowGroupsDrop.value.value = null;
  }
);
watch(
  () => colGroupsDrop.value.value,
  (value) => {
      ...
      <form @submit.prevent="submit" id="form-create-tabel">
        <div class="form-group">
              ...
              <div class="mb-3" v-if="true">
                ...
                >
                  ...
                  <tbody v-if="form.id_turtahun">
                    <tr v-for="(node, index) in turtahunListFetched" :key="index">
                      <td>{{ index + 1 }}</td>
                      <td>{{ node.tipe }}</td>
                      <td>{{ node.label }}</td>
                    </tr>
          ...
          <div class="card">
            <div class="card-body text-right">
              <div @click="buildValue" class="btn bg-info-fordone">
                <font-awesome-icon icon="fa fa-save" /> Buat Tabel
              </div>
            </div>
          </div>
        </div>
        <Teleport to="body">
          <ModalBs
            ...
            @close="previewModalStatus = false"
            :title="'Preview Tabel'"
          >
            <template #modalBody>
          ...
          <ModalBs
            :-modal-status="errorModalStatus"
            @close="errorModalStatus = false"
            :title="'Daftar Error'"
          >
            <template #modalBody>
              <ul>
                <li
                  class="text-danger"
                  v-for="(node, index) in page.props.errors"
                  :key="index"
                >
                  {{ node }}
                </li>
