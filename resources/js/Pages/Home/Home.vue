...
const submit = async () => {
  ...
  if (kecamatanSelected.value.length > 0) {
    let parentDeleted = kecamatanSelected.value.map(
      (obj) => obj.wilayah_fullcode.substring(0, 4) + "000000"
    );
    searchWilayah = [...searchWilayah, ...kecamatanSelected.value];
    searchWilayah = searchWilayah.filter(
      (item) => !parentDeleted.includes(item.wilayah_fullcode)
    );
  }
  if (desaSelected.value.length > 0) {
    let parentDeleted = desaSelected.value.map(
      (obj) => obj.wilayah_fullcode.substring(0, 7) + "000"
    );
  ...
  ArrayBigObjects.value = [
    ...
    { key: "category", valueFilter: form.category },
  ];
  try {
...
const reset = () => {
  ...
  if (wilayahCheckBox.value.length > 0)
    wilayahCheckBox.value = Array(wilayahCheckBox.value.length).fill(false);
  if (kecamatanCheckBox.value.length > 0)
    kecamatanCheckBox.value = Array(kecamatanCheckBox.value.length).fill(false);
  if (desaCheckBox.value.length > 0)
    desaCheckBox.value = Array(desaCheckBox.value.length).fill(false);
  if (subjectCheckBox.value.length > 0)
    subjectCheckBox.value = Array(subjectCheckBox.value.length).fill(false);
...
const handleChangeCheckBoxKec = (index, object) => {
  ...
  if (object[index]) {
    ...
    removeDesa.forEach((item) => {
      const theIndex = visibleDesa.value.findIndex((x) => x.parent_code === item);
      if (theIndex !== -1) visibleDesa.value[theIndex].value = false;
      desaCheckBox.value[theIndex] = false;
    });
...
const handleChangeCheckBoxKab = function (index, object) {
  ...
  if (object[index]) {
    ...
    desaRemove.forEach((item) => {
      const theIndex = visibleDesa.value.findIndex(
        (x) => x.parent_code === item.parent_code
      );
      if (theIndex !== -1) visibleDesa.value[theIndex].value = false;
      desaCheckBox.value[theIndex] = false;
    });
    removeKecamatan.forEach((item) => {
      const theIndex = visibleKecs.value.findIndex((x) => x.parent_code === item);
      if (theIndex !== -1) visibleKecs.value[theIndex].value = false;
      kecamatanCheckBox.value[theIndex] = false;
    });
          ...
          <div class="card-body d-flex align-items-start flex-column">
            ...
            <div class="p-2" id="header-banner-text">
              <div>
                ...
                <p class="lead">
                  Temukan data Pemerintah Provinsi Sulawesi Utara dengan mudah!
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="row p-0">
          ...
          <form id="search-tabel">
            ...
            <div class="card shadow mb-2">
              ...
              <div class="card-body">
                <div class="card-large-homepage">
                  <div
                    v-if="page.props.wilayahs.length > 0"
                    v-for="(node, index) in page.props.wilayahs"
                    class="my-2 d-flex"
                  >
                    <input
                      :id="'checkbox-wilayah-' + index"
                      @change="handleChangeCheckBoxKab(index, wilayahCheckBox)"
                      type="checkbox"
                      ref="checkbox"
                      v-model="wilayahCheckBox[index]"
                      :value="index"
                    />
                    <label
                      :for="'checkbox-wilayah-' + index"
                      class="ml-3 mb-0 text-capitalize"
                      :title="node.label"
                      >{{ node.label }}</label
                    >
                  </div>
                  <div class="my-2" v-else>
                    <label class="ml-3 mb-0 text-capitalize" data-target="">
                      Data belum ada yang final
                    </label>
                  </div>
                </div>
              </div>
                ...
                <div class="card-small-homepage">
                  <template v-for="(node, index) in page.props.desa" :key="index">
                    <div v-if="isVisibleDesa(node.parent_code)" class="d-flex my-2" id="">
                      <input
                        type="checkbox"
                        :id="'checkbox-desa-' + index"
                        class="desa-checkbox"
                        v-model="desaCheckBox[index]"
                      />
                      <label
                        class="ml-3 mb-0 text-capitalize"
                        :for="'checkbox-desa-' + index"
                        :title="node.label"
                      >
                        {{ node.label }}
                      </label>
            ...
            <div class="card shadow mb-2">
              ...
              <div
                class="card-header py-3 d-flex flex-row align-items-center justify-content-between"
              >
                <h6 class="m-0 font-weight-bold heading-card">
                  Berdasarkan Produsen Data
                </h6>
              </div>
              <!-- Card Body -->
              <div class="card-body">
            ...
            <div class="card shadow mb-2">
              ...
              <div class="card-body">
                <div class="card-large-homepage">
                  <div
                    v-if="page.props.subjects.length > 0"
                    v-for="(node, index) in page.props.subjects"
                    class="my-2 d-flex"
                  >
                    <input
                      type="checkbox"
                      ref="checkbox"
                      v-model="subjectCheckBox[index]"
                      :value="index"
                    />
                    <label
                      @click="toggleCheck(index, subjectCheckBox)"
                      class="ml-3 mb-0 text-capitalize"
                      :title="node.label"
                      >{{ node.label }}</label
                    >
        ...
        <div class="col-xl-8 col-lg-5">
          <div class="card shadow" id="tabel-list">
            <TabelListTimeline
              :count-tabels="countDataFiltered"
              :data="dataFiltered"
              :-array-filter="ArrayBigObjects"
              :update-result="updateResult"
              @update:update-result="updateResultResults"
              :page-number="pageNumber"
              @update:updatePageNumber="updatePageNumber"
              :reset-data="resetComponent"
