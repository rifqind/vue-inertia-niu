...
const getDashboard = async function () {
  ...
  try {
    const response = await axios.get(
      `/getDashboard/${yearDrop.value.value}/${kabsDrop.value.value}`
    );
...
<template>
  ...
  <GeneralLayout>
    <div class="d-flex mb-2">
      ...
      <button @click.prevent="getDashboard" type="submit" class="btn bg-info-fordone">
      ...
      <div class="card col" id="box-dua">
        <div class="card-body">
          <div class="row no-gutters align-items-center">
            ...
            <div class="col-auto align-middle">
              <font-awesome-icon size="2x" icon="fa-solid fa-pen-nib" class="text-dua" />
            </div>
          </div>
        </div>
      </div>
