...
const loadKecamatans = async (valueKabs) => {
  if (valueKabs) {
    try {
      ...
      let tempt = kabsDrop.value.options.filter((x) => x.value.includes(valueKabs));
      let parents = {
        label: tempt[0].label,
        value: tempt[0].value,
      };
  ...
  <GeneralLayout>
    <div class="container-fluid p-0">
      <div class="mb-2 d-flex flex-wrap align-items-center">
        <div class="h4 mb-2 mb-md-0 flex-grow-1">Master Tabel</div>
        <button
          class="btn mb-2 mb-md-0 bg-success-fordone mr-2"
          title="Download"
          @click="downloadRoute()"
        >
      ...
      >
        <thead>
          <tr class="bg-info-fordone">
            <th
              class="first-column text-center th-order align-middle"
              @click="clickSortProperties(tables, 'number')"
            >
              No.
            </th>
            <th
              class="text-center align-middle th-order tabel-width-20"
              @click="clickToOrder('tabels.label')"
            >
              Nama Tabel
            </th>
            <th
            ...
            <th class="text-center align-middle tabel-width-15">Daftar Kolom</th>
            <th class="text-center align-middle">Daftar Baris</th>
            <th class="text-center align-middle tabel-width-10">Daftar Tahun</th>
            <th
              class="text-center align-middle th-order"
              @click="clickToOrder('tabels.updated_at')"
            >
              Terakhir di-update
            </th>
            <th class="text-center align-middle tabel-width-5">Tambah Tahun</th>
          </tr>
          <tr class="">
        ...
        <tbody>
          <tr
            ...
            v-for="(table, index) in paginatedData"
            :key="index"
          >
            ...
            <td class="align-middle">
              <template v-for="(col, colIndex) in table.columns" :key="colIndex">
                ...
                <template v-else>
                  <span
                    v-if="indexExpandedCol[index] || openColList(colIndex)"
            ...
            <td class="align-middle">
              <template v-for="(row, rowIndex) in table.rowInputs" :key="rowIndex">
                <template v-if="row.wilayah_fullcode">
                  <span
                    v-if="indexExpandedRow[index] || openRowList(rowIndex)"
                    class="badge mr-1 badge-info"
                  >
                    {{ row.label }}
                  </span>
              ...
              <span
                v-if="table.rowInputs[0].wilayah_fullcode || table.rowInputs.length > 5"
                class="badge mr-1 badge-info"
                @click="openOtherRow(index)"
              >
                <font-awesome-icon
                  v-if="!indexExpandedRow[index]"
                  icon="fa-solid fa-angle-down"
                />
                <font-awesome-icon
                  v-if="indexExpandedRow[index]"
            ...
            <td class="align-middle">
              <span
                v-for="(node, index) in table.tahuns"
                :key="index"
                class="badge mr-1 badge-info"
              >
                {{ node }}
            ...
            <td class="text-center align-middle deleted">
              ...
              <a
                class="edit-pen mx-1"
                @click.prevent="patchTabel(table.id, table.id_statustables)"
              >
              ...
              >
                <font-awesome-icon
                  icon="fa-solid fa-trash-can icon-trash-color"
                  title="Hapus"
                />
              </a>
              <a
          ...
          <tr v-else>
            <td colspan="8" class="text-center align-middle">Tidak ada data</td>
          </tr>
        </tbody>
      </table>
    </div>
    <Teleport to="body">
      ...
      >
        ...
        <template #modalFunction>
          <button
            ...
            @click.prevent="GoDownload('tabel-master', downloadTitle)"
          >
            Simpan
          </button>
        </template>
      ...
      <ModalBs
        :-modal-status="duplicateModalStatus"
        ...
        <template #modalBody>
          <div class="form-group">
            ...
            <div class="mb-3">
              ...
              <Multiselect
                v-model="duplicateForm.produsen"
                :value="duplicateForm.produsen"
                :options="ProdusenFetched"
                  ...
                  <tbody v-if="tingkatanDrop.value">
                    ...
                    <tr v-else>
                      <td colspan="4" class="text-center">
                        Belum ada daftar baris yang tersedia
                      </td>
                    </tr>
                  </tbody>
                </table>
